...
assert(f('', 'a?') == '')
assert(f('é', 'é?') == 'é')
assert(f('ébl', 'é?b?l?') == 'ébl')
assert(f('  ébl', 'é?b?l?') == '')
-- FIXME(backtrack-capture): chained optional quantifiers (`a?a?a`)
-- don't backtrack correctly. Blocked.
-- assert(f('aa', '^aa?a?a') == 'aa')
-- FIXME(charclass-exclude): `[^]]` (class excluding `]`) matches the
-- wrong offset in multibyte contexts. Blocked.
-- assert(f(']]]éb', '[^]]') == 'é')
assert(f("0alo alo", "%x*") == "0a")
assert(f("alo alo", "%C+") == "alo alo")
print('+')

-- FIXME(backtrack-capture): `(..*) %1` style capture+backreference
-- with position captures (f1 rewrites %1â†’%2) fails to backtrack the
-- quantified capture. Blocked.
...
assert(string.find("(élo)", "%(é") == 1)
local a, b, c, d, e = string.match("élo alo", "^(((.).).* (%w*))$")
assert(a == 'élo alo' and b == 'él' and c == 'é' and d == 'alo' and e == nil)
a, b, c, d  = string.match('0123456789', '(.+(.?)())')
assert(a == '0123456789' and b == '' and c == 11 and d == nil)
print('+')

assert(string.gsub('élo élo', 'é', 'x') == 'xlo xlo')
assert(string.gsub('alo élo  ', ' +$', '') == 'alo élo')  -- trim
assert(string.gsub('  alo alo  ', '^%s*(.-)%s*$', '%1') == 'alo alo')  -- double trim
assert(string.gsub('alo  alo  \n 123\n ', '%s+', ' ') == 'alo alo 123 ')
t = "abé d"
a, b = string.gsub(t, '(.)', '%1@')
assert('@'..a == string.gsub(t, '', '@') and b == 5)
a, b = string.gsub('abéd', '(.)', '%0@', 2)
assert(a == 'a@b@éd' and b == 2)
assert(string.gsub('alo alo', '()[al]', '%1') == '12o 56o')
assert(string.gsub("abc=xyz", "(%w*)(%p)(%w+)", "%3%2%1-%0") ==
              "xyz=abc-abc=xyz")
assert(string.gsub("abc", "%w", "%1%0") == "aabbcc")
assert(string.gsub("abc", "%w+", "%0%1") == "abcabc")
assert(string.gsub('ééé', '$', '\0éé') == 'ééé\0éé')
