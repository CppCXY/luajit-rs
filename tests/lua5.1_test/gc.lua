...
print("114 step =", collectgarbage("step", 1000000)); print("114 eq =", collectgarbage("step", 1000000) == true); assert(collectgarbage("step", 1000000) == true)
...
x,y,z=nil
collectgarbage()
local nk = next(a); print("197 next =", nk, "len", #tostring(nk)); assert(nk == string.rep("$", 11))
...
assert(s==11)
collectgarbage()
-- FIXME(two-cycle-removal): finalized userdata keys are only dropped
-- after several extra cycles. Stale frame slots (compiler temps holding
-- the userdata) keep the key chain alive: the marked userdata resurrects
-- its metatable (and via metatable.a / the closure upvalues the whole
-- a1 key set), delaying the weak-table clear past the expected two
-- cycles. Needs a marking-side fix (e.g. finalized userdata must not
-- re-mark its metatable) or compiler KNIL coverage for stale temps.
