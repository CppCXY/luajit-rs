...
use crate::bc::{BCOp, bc_isret, bc_j, bc_op, setbc_op};
use crate::gc::GcPtr;
...
fn trace_stop(g: &mut GlobalState, mut rec: Box<Record>, linktype: TraceLink) {
    ...
    // Park the start instruction's hot counter: without JLOOP patching the
    // loop keeps interpreting, and the registry check in trace_start would
    // otherwise run every ~hotloop iterations.
    js.hotcount_set(bc_addr(trace.startpt, trace.startpc + 1), HOTCOUNT_PARKED);
...
mod tests {
    ...
    #[test]
    fn numeric_forl_records_a_looping_trace() {
        let mut lua = Lua::new();
        ...
        let tr = g
            .jit
            .find_root_trace(pt, find_op(pt, BCOp::FORL).unwrap())
            .and_then(|no| g.jit.trace[no as usize].as_deref())
            .expect("root trace must exist");
        // A looping root trace, linked to itself.
        ...
        // At least the entry snapshot and the loop-edge snapshot.
        assert!(tr.snap.len() >= 2);
        // The FORL keeps interpreting (no JLOOP patching yet).
        assert!(find_op(pt, BCOp::FORL).is_some());
        // No penalty was assessed for the start pc.
        let key = bc_addr(pt, find_op(pt, BCOp::FORL).unwrap());
    ...
    fn while_loop_records_via_loop_root() {
        ...
        let g = lua.global();
        let pc = find_op(pt, BCOp::LOOP).unwrap();
        let no = g.jit.find_root_trace(pt, pc).expect("LOOP root trace");
        let tr = g.jit.trace[no as usize].as_deref().unwrap();
        assert_eq!(tr.linktype, TraceLink::Loop);
