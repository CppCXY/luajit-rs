...
impl Emit {
    ...
    fn nop(&mut self) {
        self.u32(0xD503_201F);
    }
    fn brk(&mut self, imm: u16) {
        self.u32(0xD420_0000 | ((imm as u32) << 5));
    }

    // ── move wide ──
    fn movz(&mut self, xd: u8, imm: u16, sh: u8) {
        let h = (sh / 16) as u32;
        self.u32(0xD280_0000 | (h << 21) | ((imm as u32) << 5) | (xd as u32));
    }
    fn movk(&mut self, xd: u8, imm: u16, sh: u8) {
        let h = (sh / 16) as u32;
        self.u32(0xF280_0000 | (h << 21) | ((imm as u32) << 5) | (xd as u32));
    }
    fn mov64(&mut self, xd: u8, imm: u64) {
        self.movz(xd, imm as u16, 0);
        if imm >> 16 != 0 {
            self.movk(xd, (imm >> 16) as u16, 16);
        }
        if imm >> 32 != 0 {
            self.movk(xd, (imm >> 32) as u16, 32);
        }
        if imm >> 48 != 0 {
            self.movk(xd, (imm >> 48) as u16, 48);
        }
    }
    fn mov32(&mut self, wd: u8, imm: u32) {
        self.movz(wd, imm as u16, 0);
        if imm >> 16 != 0 {
            self.movk(wd, (imm >> 16) as u16, 16);
        }
    }

    // ── branch ──
    fn b_cond(&mut self, cc: u32, off: i32) {
        self.u32(0x5400_0000 | (((off as u32) & 0x7FFFF) << 5) | cc);
    }
    fn b(&mut self, off: i32) {
        self.u32(0x1400_0000 | ((off as u32) & 0x3FF_FFFF));
    }
    fn br(&mut self, rn: u8) {
        self.u32(0xD61F_0000 | ((rn as u32) << 5));
    }
    fn ret(&mut self) {
        self.u32(0xD65F_03C0);
    }
    fn blr(&mut self, rn: u8) {
        self.u32(0xD63F_0000 | ((rn as u32) << 5));
    }

    // ── DP register (64-bit unless noted) ──
    fn add_rr(&mut self, rd: u8, rn: u8, rm: u8) {
        self.u32(0x8B00_0000 | ((rm as u32) << 16) | ((rn as u32) << 5) | (rd as u32));
    }
    fn sub_rr(&mut self, rd: u8, rn: u8, rm: u8) {
        self.u32(0xCB00_0000 | ((rm as u32) << 16) | ((rn as u32) << 5) | (rd as u32));
    }
    fn and_rr(&mut self, rd: u8, rn: u8, rm: u8) {
        self.u32(0x8A00_0000 | ((rm as u32) << 16) | ((rn as u32) << 5) | (rd as u32));
    }
    fn orr_rr(&mut self, rd: u8, rn: u8, rm: u8) {
        self.u32(0xAA00_0000 | ((rm as u32) << 16) | ((rn as u32) << 5) | (rd as u32));
    }
    fn eor_rr(&mut self, rd: u8, rn: u8, rm: u8) {
        self.u32(0xCA00_0000 | ((rm as u32) << 16) | ((rn as u32) << 5) | (rd as u32));
    }
    fn mvn64(&mut self, rd: u8, rm: u8) {
        self.u32(0xAA20_03E0 | ((rm as u32) << 16) | (rd as u32));
    }
    // 32-bit forms
    fn and_w(&mut self, wd: u8, wn: u8, wm: u8) {
        self.u32(0x0A00_0000 | ((wm as u32) << 16) | ((wn as u32) << 5) | (wd as u32));
    }
    fn orr_w(&mut self, wd: u8, wn: u8, wm: u8) {
        self.u32(0x2A00_0000 | ((wm as u32) << 16) | ((wn as u32) << 5) | (wd as u32));
    }
    fn eor_w(&mut self, wd: u8, wn: u8, wm: u8) {
        self.u32(0x4A00_0000 | ((wm as u32) << 16) | ((wn as u32) << 5) | (wd as u32));
    }
    ...
    fn sub_imm(&mut self, rd: u8, rn: u8, imm: u32) {
        self.u32(0xD100_0000 | ((imm & 0xFFF) << 10) | ((rn as u32) << 5) | (rd as u32));
    }
    fn cmp_rr(&mut self, rn: u8, rm: u8) {
        self.u32(0xEB00_001F | ((rm as u32) << 16) | ((rn as u32) << 5));
    }
    fn cmp_rr_w(&mut self, wn: u8, wm: u8) {
        self.u32(0x6B00_001F | ((wm as u32) << 16) | ((wn as u32) << 5));
    }
    fn cmp_imm(&mut self, rn: u8, imm: u32) {
        self.u32(0xF100_001F | ((imm & 0xFFF) << 10) | ((rn as u32) << 5));
    }

    fn lsl_rr(&mut self, rd: u8, rn: u8, rm: u8) {
        self.u32(0x9AC0_2000 | ((rm as u32) << 16) | ((rn as u32) << 5) | (rd as u32));
    }
    fn lsr_rr(&mut self, rd: u8, rn: u8, rm: u8) {
        self.u32(0x9AC0_2400 | ((rm as u32) << 16) | ((rn as u32) << 5) | (rd as u32));
    }
    fn asr_rr(&mut self, rd: u8, rn: u8, rm: u8) {
        self.u32(0x9AC0_2800 | ((rm as u32) << 16) | ((rn as u32) << 5) | (rd as u32));
    }
    ...
    fn lsr_w(&mut self, wd: u8, wn: u8, wm: u8) {
        self.u32(0x1AC0_2400 | ((wm as u32) << 16) | ((wn as u32) << 5) | (wd as u32));
    }
    fn asr_w(&mut self, wd: u8, wn: u8, wm: u8) {
        self.u32(0x1AC0_2800 | ((wm as u32) << 16) | ((wn as u32) << 5) | (wd as u32));
    }
    ...
    fn stp(&mut self, rt1: u8, rt2: u8, rn: u8, off: i32) {
        let o = ((off / 8) as u32) & 0x7F;
        self.u32(
            0xA900_0000 | (o << 15) | ((rt2 as u32) << 10) | ((rn as u32) << 5) | (rt1 as u32),
        );
    }
    fn ldp(&mut self, rt1: u8, rt2: u8, rn: u8, off: i32) {
        let o = ((off / 8) as u32) & 0x7F;
        self.u32(
            0xA940_0000 | (o << 15) | ((rt2 as u32) << 10) | ((rn as u32) << 5) | (rt1 as u32),
        );
    }
    fn stp_d(&mut self, dt1: u8, dt2: u8, rn: u8, off: i32) {
        let o = ((off / 8) as u32) & 0x7F;
        self.u32(
            0x6D00_0000 | (o << 15) | ((dt2 as u32) << 10) | ((rn as u32) << 5) | (dt1 as u32),
        );
    }
    fn ldp_d(&mut self, dt1: u8, dt2: u8, rn: u8, off: i32) {
        let o = ((off / 8) as u32) & 0x7F;
        self.u32(
            0x6D40_0000 | (o << 15) | ((dt2 as u32) << 10) | ((rn as u32) << 5) | (dt1 as u32),
        );
    }
    ...
    fn ldr(&mut self, rt: u8, rn: u8, off: i32) {
        self.u32(
            0xF940_0000 | ((((off / 8) as u32) & 0xFFF) << 10) | ((rn as u32) << 5) | (rt as u32),
        );
    }
    fn str(&mut self, rt: u8, rn: u8, off: i32) {
        self.u32(
            0xF900_0000 | ((((off / 8) as u32) & 0xFFF) << 10) | ((rn as u32) << 5) | (rt as u32),
        );
    }
    fn ldr_w(&mut self, rt: u8, rn: u8, off: i32) {
        self.u32(
            0xB940_0000 | ((((off / 4) as u32) & 0xFFF) << 10) | ((rn as u32) << 5) | (rt as u32),
        );
    }
    fn str_w(&mut self, rt: u8, rn: u8, off: i32) {
        self.u32(
            0xB900_0000 | ((((off / 4) as u32) & 0xFFF) << 10) | ((rn as u32) << 5) | (rt as u32),
        );
    }
    fn ldr_d(&mut self, dt: u8, rn: u8, off: i32) {
        self.u32(
            0xFD40_0000 | ((((off / 8) as u32) & 0xFFF) << 10) | ((rn as u32) << 5) | (dt as u32),
        );
    }
    fn str_d(&mut self, dt: u8, rn: u8, off: i32) {
        self.u32(
    ...
    // ── FP ops ──
    fn fmov_dd(&mut self, dd: u8, dn: u8) {
        self.u32(0x1E60_4000 | ((dn as u32) << 5) | (dd as u32));
    }
    fn fmov_gpr(&mut self, xd: u8, dn: u8) {
        self.u32(0x9E66_0000 | ((dn as u32) << 5) | (xd as u32));
    }
    fn fmov_fp(&mut self, dd: u8, xn: u8) {
        self.u32(0x9E67_0000 | ((xn as u32) << 5) | (dd as u32));
    }
    fn fadd(&mut self, dd: u8, dn: u8, dm: u8) {
        self.u32(0x1E60_2800 | ((dm as u32) << 16) | ((dn as u32) << 5) | (dd as u32));
    }
    fn fsub(&mut self, dd: u8, dn: u8, dm: u8) {
        self.u32(0x1E60_3800 | ((dm as u32) << 16) | ((dn as u32) << 5) | (dd as u32));
    }
    fn fmul(&mut self, dd: u8, dn: u8, dm: u8) {
        self.u32(0x1E60_0800 | ((dm as u32) << 16) | ((dn as u32) << 5) | (dd as u32));
    }
    fn fdiv(&mut self, dd: u8, dn: u8, dm: u8) {
        self.u32(0x1E60_1800 | ((dm as u32) << 16) | ((dn as u32) << 5) | (dd as u32));
    }
    fn fneg_d(&mut self, dd: u8, dn: u8) {
        self.u32(0x1E61_4000 | ((dn as u32) << 5) | (dd as u32));
    }
    fn fabs_d(&mut self, dd: u8, dn: u8) {
        self.u32(0x1E60_C000 | ((dn as u32) << 5) | (dd as u32));
    }
    fn fsqrt(&mut self, dd: u8, dn: u8) {
        self.u32(0x1E61_C000 | ((dn as u32) << 5) | (dd as u32));
    }
    fn frintm(&mut self, dd: u8, dn: u8) {
        self.u32(0x1E65_4000 | ((dn as u32) << 5) | (dd as u32));
    }
    fn frintp(&mut self, dd: u8, dn: u8) {
        self.u32(0x1E64_C000 | ((dn as u32) << 5) | (dd as u32));
    }
    fn frintz(&mut self, dd: u8, dn: u8) {
        self.u32(0x1E65_C000 | ((dn as u32) << 5) | (dd as u32));
    }
    fn fcmp(&mut self, dn: u8, dm: u8) {
        self.u32(0x1E60_2000 | ((dm as u32) << 16) | ((dn as u32) << 5));
    }
    fn scvtf_w(&mut self, dd: u8, wn: u8) {
        self.u32(0x1E62_0000 | ((wn as u32) << 5) | (dd as u32));
    }
    fn scvtf_x(&mut self, dd: u8, xn: u8) {
        self.u32(0x9E62_0000 | ((xn as u32) << 5) | (dd as u32));
    }
    fn ucvtf_w(&mut self, dd: u8, wn: u8) {
        self.u32(0x1E63_0000 | ((wn as u32) << 5) | (dd as u32));
    }
    fn ucvtf_x(&mut self, dd: u8, xn: u8) {
        self.u32(0x9E63_0000 | ((xn as u32) << 5) | (dd as u32));
    }
    fn fcvtzs_w(&mut self, wd: u8, dn: u8) {
        self.u32(0x1E78_0000 | ((dn as u32) << 5) | (wd as u32));
    }
    fn fcvtzs_x(&mut self, xd: u8, dn: u8) {
        self.u32(0x9E78_0000 | ((dn as u32) << 5) | (xd as u32));
    }

    // fcsel Dd, Dn, Dm, cond
    fn fcsel(&mut self, dd: u8, dn: u8, dm: u8, cc: u32) {
        self.u32(0x1E20_0C00 | ((dm as u32) << 16) | (cc << 12) | ((dn as u32) << 5) | (dd as u32));
    }
}
...
mod cond {
    ...
    pub const CS: u32 = 0x2;
    pub const CC: u32 = 0x3;
    pub const MI: u32 = 0x4;
    pub const PL: u32 = 0x5;
    pub const VS: u32 = 0x6;
    pub const VC: u32 = 0x7;
    pub const HI: u32 = 0x8;
    pub const LS: u32 = 0x9;
...
impl<'a> Asm<'a> {
    ...
    fn fetch_fp(&mut self, r: IRRef, pinned: u32) -> Result<u8, TraceError> {
        ...
        if r >= REF_BIAS {
            self.code.ldr_d(rg, RENV, Self::env_ofs(r));
            self.owner[rg as usize] = Owner::Ins(r);
            self.loc[Self::iidx(r)] = Some(rg);
        } else {
            self.code.mov64(RSCRATCH, exec::const_bits(&self.tr.ir, r));
    ...
    fn gpr_load_ref(&mut self, rd: u8, r: IRRef) {
        if r == REF_BIAS {
            ...
            self.code.mov64(rd, exec::const_bits(&self.tr.ir, r));
        }
    }
    fn def(&mut self, d: u8) {
        ...
        self.loc[i] = Some(d);
        if self.needs_env[i] {
            self.code.str_d(d, RENV, Self::env_ofs(self.cur));
            self.env_valid[i] = true;
        }
    }
    ...
    fn exit_flush_set(&self, snapidx: usize) -> Vec<(u8, IRRef)> {
        ...
        for sn in &self.tr.snapmap[ofs..ofs + snap.nent as usize] {
            ...
            let i = Self::iidx(r);
            if self.env_valid[i] || f.iter().any(|&(_, fr)| fr == r) {
                continue;
            }
            if let Some(rg) = self.loc[i] {
                f.push((rg, r));
            }
        }
        f
    ...
    fn guard(&mut self, cc: u32) {
        let s = self.make_stub(false);
        let pos = self.code.len();
        self.code.b_cond(cc, 0);
        self.fixups.push((pos, s));
    }
    fn guard_gc(&mut self, cc: u32) {
        let s = self.make_stub(true);
        let pos = self.code.len();
        self.code.b_cond(cc, 0);
        self.fixups.push((pos, s));
    }
    ...
    fn asm_uload(&mut self, ins: &IRIns) -> Result<(), TraceError> {
        ...
        if ty == IRT_NIL {
            self.code.ldr(RSCRATCH2, RSCRATCH, 0);
            self.code.mov64(RSCRATCH3, (-1i64) as u64);
            self.code.cmp_rr(RSCRATCH2, RSCRATCH3);
            self.guard(cond::NE);
        } else if ty <= IRT_TRUE {
    ...
    fn asm_aload(&mut self, ins: &IRIns) -> Result<(), TraceError> {
        ...
        if self.loop_pos.is_some() {
            self.helper_call(
                exec::jit_tget as *const () as usize as u64,
    ...
    fn key_provably_int(&self, keyref: IRRef) -> bool {
        ...
        match ki.op() {
            IROp::ADD => (ki.op2 as u32) < REF_BIAS,
            IROp::PHI => {
    ...
    fn asm_arith(&mut self, ins: &IRIns) -> Result<(), TraceError> {
        ...
        match op {
            ...
            IROp::MIN => {
                ...
                self.code.fcsel(d, d, rhs, cond::GT);
            } // if d > rhs pick rhs (min)
            IROp::MAX => {
                self.code.fcmp(d, rhs);
                self.code.fcsel(d, d, rhs, cond::MI);
            } // if d < rhs pick rhs (max)
    ...
    fn tail_restore(&mut self, snapidx: usize) {
        ...
        for &sn in &self.tr.snapmap[ofs..ofs + snap.nent as usize] {
            ...
            if rref >= REF_BIAS {
                ...
                self.code
                    .mov64(RSCRATCH, exec::const_bits(&self.tr.ir, rref));
                self.code.str_safe(RSCRATCH, RBASE, disp);
            }
        }
    }
    ...
    fn emit(mut self) -> Result<(McodeArea, u32, Vec<(u32, u32)>), TraceError> {
        ...
        for i in 0..SAVED_GPR_PAIRS {
            ...
            self.code
                .stp(19 + i as u8 * 2, 19 + i as u8 * 2 + 1, 29, off);
        }
        for i in 0..SAVED_FP_PAIRS {
            let off = 16 + SAVED_GPR_PAIRS as i32 * 16 + (i as i32) * 16;
            self.code
                .stp_d(8 + i as u8 * 2, 8 + i as u8 * 2 + 1, 29, off);
        }
        ...
        while r < nins {
            ...
            match ins.op() {
                ...
                IROp::LOOP => {
                    if self.loop_pos.is_none() {
                        ...
                        self.phi_homes = self
                            .phis
                            .iter()
                            .filter_map(|p| self.reg_of(p.lref).map(|rg| (p.lref, rg)))
                            .collect();
                    }
                }
                IROp::PHI => {}
                ...
                IROp::POW => {
                    self.helper_call(
                        exec::jit_pow as *const () as usize as u64,
                        &[ins.op1 as IRRef, ins.op2 as IRRef],
                    );
                    self.ff_result(&ins)?;
                }
            ...
            if let Some(lp) = self.loop_pos {
                ...
                for p in self.phis.iter().filter(|p| !p.num) {
                    if p.rref >= REF_BIAS {
                        ...
                        self.code
                            .mov64(RSCRATCH, exec::const_bits(&self.tr.ir, p.rref));
                        self.code.str(RSCRATCH, RENV, Self::env_ofs(p.lref));
                        self.env_valid[Self::iidx(p.lref)] = true;
                    }
                }
                ...
                for rg in 0..NREG as u8 {
                    ...
                    match so {
                        ...
                        Owner::Konst(k) => {
                            self.code.mov64(RSCRATCH, exec::const_bits(&self.tr.ir, k));
                            self.code.fmov_fp(rg, RSCRATCH);
                        }
            ...
            && let Some(target) = self.link
        {
            ...
            for (rg, fr) in self.exit_flush_set(lastsnap) {
                self.code.str_d(rg, RENV, Self::env_ofs(fr));
            }
        ...
        for i in 0..SAVED_GPR_PAIRS {
            let off = 16 + (i as i32) * 16;
            self.code
                .ldp(19 + i as u8 * 2, 19 + i as u8 * 2 + 1, 29, off);
        }
        for i in 0..SAVED_FP_PAIRS {
            let off = 16 + SAVED_GPR_PAIRS as i32 * 16 + (i as i32) * 16;
            self.code
                .ldp_d(8 + i as u8 * 2, 8 + i as u8 * 2 + 1, 29, off);
        }
...
fn disasm_a64(w: u32, _addr: usize) -> String {
    ...
    if w & 0xFF00_0000 == 0xD100_0000 {
        return format!("sub x{}, x{}, #{}", rd, rn, imm12);
    }
    if w & 0xFF00_0000 == 0x9100_0000 {
        return format!("add x{}, x{}, #{}", rd, rn, imm12);
    }
    if w & 0xFFE0_0000 == 0x8B00_0000 {
        return format!("add x{}, x{}, x{}", rd, rn, rm);
    }
    if w & 0xFFE0_0000 == 0xCB00_0000 {
        return format!("sub x{}, x{}, x{}", rd, rn, rm);
    }
    if w & 0xFFE0_0000 == 0x8A00_0000 {
        return format!("and x{}, x{}, x{}", rd, rn, rm);
    }
    if w & 0xFFE0_0000 == 0x0A00_0000 {
        return format!("and w{}, w{}, w{}", rd, rn, rm);
    }
    if w & 0xFFE0_0000 == 0xAA00_0000 {
        return format!("orr x{}, x{}, x{}", rd, rn, rm);
    }
    if w & 0xFFE0_0000 == 0x2A00_0000 {
        return format!("orr w{}, w{}, w{}", rd, rn, rm);
    }
    if w & 0xFFE0_0000 == 0xCA00_0000 {
        return format!("eor x{}, x{}, x{}", rd, rn, rm);
    }
    if w & 0xFFE0_0000 == 0xEB00_001F {
        return format!("cmp x{}, x{}", rn, rm);
    }
    if w & 0xFC00_0000 == 0xA900_0000 {
        let o = ((w >> 15) & 0x7F) * 8;
        return format!(
            "stp x{}, x{}, [x{}, #{}]",
            rd,
            rm >> 5, /*rt2 is bits 14-10*/
            rn,
            o
        );
    }
    ...
    if w & 0xFC00_0000 == 0xA940_0000 {
        let o = ((w >> 15) & 0x7F) * 8;
        return format!("ldp x{}, x{}, [x{}, #{}]", rd, rt2, rn, o);
    }
    ...
    if w & 0xFFC0_0000 == 0xFD400000 {
        return format!("ldr d{}, [x{}, #{}]", rd, rn, imm12 * 8);
    }
    if w & 0xFFC0_0000 == 0xFD000000 {
        return format!("str d{}, [x{}, #{}]", rd, rn, imm12 * 8);
    }
    if w & 0xFFC0_0000 == 0xB9400000 {
        return format!("ldr w{}, [x{}, #{}]", rd, rn, imm12 * 4);
    }
    if w & 0xFFFF_FC00 == 0xD280_0000 {
        return format!("movz x{}, #0x{:x}", rd, (w >> 5) & 0xFFFF);
    }
    if w & 0xFFFF_FC00 == 0xF280_0000 {
        return format!(
            "movk x{}, #0x{:x} lsl#{}",
            rd,
            (w >> 5) & 0xFFFF,
    ...
    if w & 0xFF20_0000 == 0x1E60_2800 {
        return format!("fadd d{}, d{}, d{}", rd, rn, rm);
    }
    if w & 0xFF20_0000 == 0x1E60_3800 {
        return format!("fsub d{}, d{}, d{}", rd, rn, rm);
    }
    if w & 0xFF20_0000 == 0x1E60_0800 {
        return format!("fmul d{}, d{}, d{}", rd, rn, rm);
    }
    if w & 0xFF20_0000 == 0x1E60_1800 {
        return format!("fdiv d{}, d{}, d{}", rd, rn, rm);
    }
    if w & 0xFF20_0000 == 0x1E60_2000 {
        return format!("fcmp d{}, d{}", rn, rm);
    }
    if w & 0xFF20_0000 == 0x1E60_4000 {
        return format!("fmov d{}, d{}", rd, rn);
    }
    if w & 0xFF20_0000 == 0x1E61_4000 {
        return format!("fneg d{}, d{}", rd, rn);
    }
    if w & 0xFC00_0000 == 0x5400_0000 {
        return format!("b.cond #{}", imm19);
    }
    ...
    if w & 0xFFFF_FC00 == 0x9E220_000 {
        return format!("scvtf d{}, w{}", rd, rn);
    }
    if w & 0xFFFF_FC00 == 0x9E380_000 {
        return format!("fcvtzs x{}, d{}", rd, rn);
    }
    if w & 0xFFFF_FC00 == 0x1E220_000 {
        return format!("scvtf d{}, w{}", rd, rn);
    }
    if w & 0xFFFF_FC00 == 0x1E380_000 {
        return format!("fcvtzs w{}, d{}", rd, rn);
    }
    if w & 0xFFFF_FC00 == 0x9E670_000 {
        return format!("fmov d{}, x{}", rd, rn);
    }
    if w == 0xD65F_03C0 {
        return "ret".into();
    }
    if w & 0xFFFF_FC00 == 0xD63F_0000 {
        return format!("blr x{}", rn);
    }
    if w & 0xFFE0_0000 == 0x6D00_0000 {
        let o = ((w >> 15) & 0x7F) * 8;
        return format!("stp d{}, d{}, [x{}, #{}]", rd, rt2, rn, o);
    }
    if w & 0xFFE0_0000 == 0x6D40_0000 {
        let o = ((w >> 15) & 0x7F) * 8;
        return format!("ldp d{}, d{}, [x{}, #{}]", rd, rt2, rn, o);
    }
    if w == 0xD503_201F {
        return "nop".into();
    }
