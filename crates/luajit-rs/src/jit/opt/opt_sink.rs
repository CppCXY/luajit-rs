...
use crate::jit::ir::{IROp, REF_FIRST, REF_NIL, tref_isk};
...
pub fn opt_sink(t: &mut GCtrace) {
    ...
    for r in REF_FIRST..nins {
        let ins = t.ir.ir(r);
        if ins.op() != IROp::TDUP && ins.op() != IROp::CNEW {
            continue;
        }
        if !ins.is_marked() {
            continue;
    ...
    for &al_ref in &allocs {
        ...
        let Some(templ) = crate::gc::GcPtr::<crate::table::LuaTable>::from_addr(
            kgc_bits & crate::value::LJ_GCVMASK,
        ) else {
            continue;
        };
        ...
        for &(lr, _) in &forwards {
            t.ir.ir_mut(lr).set_nop();
        }
        if al_ref + 1 < nins && t.ir.ir(al_ref + 1).op() == IROp::GCSTEP {
            t.ir.ir_mut(al_ref + 1).set_nop();
        }
